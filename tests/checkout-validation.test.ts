...
beforeAll(async () => {
  ...
  // Create auth users
  const ts = Date.now();
  const teacherEmail = `teacher-co-${ts}@test.suluk.dev`;
  const studentEmail = `student-co-${ts}@test.suluk.dev`;
  const parentEmail = `parent-co-${ts}@test.suluk.dev`;
  const childEmail = `child-co-${ts}@test.suluk.dev`;
