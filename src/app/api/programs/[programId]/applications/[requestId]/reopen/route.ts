...
export async function POST(request: Request, { params }: { params: Promise<{ programId: string; requestId: string }> }) {
  try {
    ...
    if (enrollmentRequest.status !== "rejected") {
      return Response.json({ error: "Only a rejected application can be reopened." }, { status: 409 });
    }

    const { error: updateError } = await supabase.from("enrollment_requests").update({ status: "pending" }).eq("id", requestId);
    if (updateError) {
    ...
    await recordFinanceAuditEvent(supabase, {
      ...
      summary: `Application reopened for ${student?.full_name || student?.email || "this student"} — back to pending review.`,
    });
    ...
    if (program && mosque) {
      void sendPushNotification(supabase, {
        ...
        body: `Your application to ${program.title} was reopened and is back under review.`,
    ...
    await logServerError(createSupabaseServiceClient(), {
      source: "programs.applications.reopen",
      message,
